...
function renderLabel({ cx, cy, midAngle, innerRadius, outerRadius, payload }: PieLabelProps) {
  if (cx == null || cy == null || midAngle == null || innerRadius == null || outerRadius == null || !payload) return null;
...
function CustomTooltip({ active, payload }: TooltipState) {
  ...
  return (
    <div className="rounded-sm border border-border bg-card px-2 py-1 text-xs shadow-sm">
  ...
  if (data.length === 0) {
    return (
      <div className="flex h-13 items-center justify-center">
        <span className="text-xs text-muted-foreground">배출 데이터 없음</span>
      </div>
    );
  }
      ...
      <div className="flex shrink-0 flex-col gap-1">
        {scopes.map(({ scope, pct }) => (
          <div key={scope} className="flex items-center gap-1 text-xs">
            <span className="font-semibold" style={{ color: SCOPE_COLORS[scope] }}>S{scope}</span>
