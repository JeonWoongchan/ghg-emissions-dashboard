            ...
            <CardContent className="space-y-5">
                        ...
                        return (
                            <div key={s} className="space-y-2">
                                ...
                                <input
                                    ...
                                    value={pct}
                                    onChange={(e) =>
                                        setReductions((prev) => ({ ...prev, [s]: Number(e.target.value) }))
                                    }
                                    className="w-full"
                                ...
                                <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                                    <span className="tabular-nums">{formatEmissions(current)} tCO₂e</span>
                                    {pct > 0 && (
                                        <>
                                            <span>→</span>
                                            ...
                                            <span className="text-success">(-{formatEmissions(saved)})</span>
                                        </>
                                    )}
                                </div>
                            </div>
                ...
                <div className="grid grid-cols-3 gap-3 rounded-lg bg-muted/40 p-4">
                    <div className="text-center">
                        <p className="text-xs text-muted-foreground">감축 후 배출량</p>
                    ...
                    <div className="text-center">
                        <p className="text-xs text-muted-foreground">감축 후 예상세</p>
                        <p className="mt-1 text-lg font-bold tabular-nums">
                            {hasReduction ? formatKrw(newTaxKrw) : '-'}
                        </p>
                        <p className="text-xs text-muted-foreground">연간 추정</p>
                    </div>
                </div>
            </CardContent>
        </Card>
